...
RSpec.describe Sidebar, :type => :model do
  ...
  it "belongs to static_pages" do
    expect(create(:left_sidebar).static_pages.count).to eq 4
    expect(create(:right_sidebar).static_pages.count).to eq 4
    expect(create(:top_sidebar).static_pages.count).to eq 4
    expect(create(:bottom_sidebar).static_pages.count).to eq 4
  end
