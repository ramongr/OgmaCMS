class Admin::StaticPagesController < ApplicationController
  ...
  def index
    @admin_static_pages = Admin::StaticPage.all
  end
  ...
  def new
    @admin_static_page = Admin::StaticPage.new
  end
  ...
  def create
    @admin_static_page = Admin::StaticPage.new(admin_static_page_params)

    respond_to do |format|
      if @admin_static_page.save
        format.html { redirect_to @admin_static_page, notice: 'Static page was successfully created.' }
        format.json { render action: 'show', status: :created, location: @admin_static_page }
      else
  ...
  def update
    respond_to do |format|
      if @admin_static_page.update(admin_static_page_params)
        format.html { redirect_to @admin_static_page, notice: 'Static page was successfully updated.' }
  ...
  private
    ...
    def set_admin_static_page
      @admin_static_page = Admin::StaticPage.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def admin_static_page_params
      params.require(:admin_static_page).permit(:title, :slug, :body)
    end
end
